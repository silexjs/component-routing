...
Routing.prototype = {
	...
	match: function(method, url) {
		...
		if(pathLength !== 0 && path[pathLength] === '/') { path = path.substr(0, pathLength); }
	...
	generate: function(route, variables) {
		variables = variables || {};
		...
		var path = route.config.path.replace(/\/\{([A-Za-z0-9_]+)\}/g, function(match, contents, offset, s) {
